...
# Konfigurasi awal halaman (hanya sekali)
st.set_page_config(
...
# ---- CSS styling ----
st.markdown("""<style> ... (CSS yang sama seperti punyamu) ... </style>""", unsafe_allow_html=True)
...
# --- HALAMAN 2: Input Data ---
    ...
    if st.button("Hitung Rata-rata Data Pengukuran"):
        try:
            if df.isnull().values.any() or (df == "").values.any():
                st.warning("⚠ Semua sel harus diisi sebelum menghitung rata-rata.")
            else:
    ...
    with col_prev:
        if st.button("⬅ Kembali ke Home"):
            prev_page()
    with col_next:
        if st.button("➡ Lanjut ke Stoikiometri"):
            next_page()

# --- HALAMAN 3: Stoikiometri ---
    ...
    if "rata_pengukuran" in st.session_state:
        ...
        if st.button("Hitung Volume & Ketidakpastian"):
            try:
                ...
                # Ketidakpastian (semua diperbaiki pangkatnya)
    ...
    if st.button("⬅ Kembali ke Reaksi Kimia"):
        prev_page()
